        ...
        <div className="flex flex-col h-full bg-white">
            <div className="p-6">
                ...
                <nav className="space-y-1">
                    ...
                    <SidebarLink icon={<LayoutDashboard size={18} />} label="Classic Dashboard" />
                    <SidebarLink icon={<ShoppingBag size={18} />} label="E-commerce" active subLinks={[]} />
...
export default function AdminPage() {
    ...
    return (
        <div className="flex min-h-screen bg-[#F9FAFB] flex-col lg:flex-row">
            {/* Desktop Sidebar */}
            <aside className="w-64 border-r bg-white flex flex-col hidden lg:flex fixed inset-y-0">
                <AdminSidebar />
            </aside>
                                        ...
                                        {loading ? (
                                            ...
                                            .map((product) => (
                                                <TableRow key={product.id} className="hover:bg-gray-50/50 transition-colors border-b border-gray-50">
                                                    ...
                                                    <TableCell className="py-4">
                                                        <span className="text-sm text-gray-600 capitalize">{product.category.replace('-', ' ')}</span>
                                                    </TableCell>
                                                    <TableCell className="py-4">
...
function SidebarLink({ icon, label, active, subLinks }: { icon: React.ReactNode, label: string, active?: boolean, subLinks?: { label: string, href: string, active?: boolean }[] }) {
